...
class RAMTester extends AnyFlatSpec with ChiselScalatestTester {
    ...
    it should "correctly writes two floats to RAM then reads them back" in {
        test(new RAM(8, 32)) { dut =>
            // write signal
            doWriteTest(dut, 32, 5.0, 7.0, 5.0, 7.0, 0, 1)

            // read signal
            doReadTest(dut, 32, 5.0, 7.0, 5.0, 7.0, 0, 1)
        }
    }
    ...
    it should "correctly write to all RAM and read it all back" in {
        test(new RAM(16, 32)) { dut =>
            for(i <- 0 until 16) {
                doWriteTest(dut, 32, i.toDouble, i.toDouble, i.toDouble, i.toDouble, i, i)
            }

            for(j <- 0 until 16) {
                doReadTest(dut, 32, j.toDouble, j.toDouble, j.toDouble, j.toDouble, j, j)
            }
        }
    }
    ...
    it should "correctly write one and read one element to the entire RAM" in {
        test(new RAM(32, 32)) { dut =>
            for (i <- 0 until 32) {
                doWriteTest(dut, 32, i.toDouble, i.toDouble, i.toDouble, i.toDouble, i, i)
                doReadTest(dut, 32, i.toDouble, i.toDouble, i.toDouble, i.toDouble, i, i)
            }
        }
    }
}